...
object ch11_TravelGuide {
  ...
  /** STEP 1 MODEL: just immutable values (reused in the whole application).
    *
    * Note that we have a single model for the whole application, but sometimes
    * it may be beneficial to have separate ones for data access and business
    * domain (see the book).
    */
  object model {
  ...
  /** STEP 2 DATA ACCESS (just an interface providing pure functions,
  ...
  object Version1 {
    def travelGuide(
      ...
      for {
        attractions <- data.findAttractions(
          attractionName,
  ...
  /** STEP 4: implementing real data access
    * @see
    *   [[ch11_QueryingWikidata]] for a simple Wikidata query using Apache Jena
    *   imperatively
    */
  private def runStep4 = {
    ...
    def execQuery(
        getConnection: IO[RDFConnection],
        query: String
    ...
    def findAttractions(
      ...
      for {
        ...
        attractions <- solutions.traverse(
          parseAttraction
        ) // or map(parseAttraction).sequence
      } yield attractions
    }
    ...
    def configuringAFunction = { // currying discussion (configuring a function)
      ...
      val f: (String, AttractionOrdering, Int) => IO[List[Attraction]] =
    ...
    def passingAQueryingBehavior = { // functions as values discussion (passing a querying behavior)
      ...
      val f: (String, AttractionOrdering, Int) => IO[List[Attraction]] =
        findAttractions(execQueryFunction)
    ...
    /** @see
      *   [[ch11_WikidataDataAccess]] for a Wikidata Sparql endpoint
      *   implementation using Apache Jena and final version of all DataAccess
      *   functions
      */

    assert(
      findAttractions("Bridge of Sighs", ByLocationPopulation, 1)
        .unsafeRunSync()
        .map(_.name) == List("Bridge of Sighs")
    )
  ...
  private def runStep5 = {
    def execQuery(connection: RDFConnection)(
        query: String
    ): IO[List[QuerySolution]] = IO.blocking(
  ...
  /** STEP 6: searching for the best travel guide requirements:
    ...
    *   - 1 point for each 10_000_000 dollars in box-office totals (all movies
    *     combined, max 15pts)
    */
  ...
  private def runVersion2 = { // handling resource release manually
    def execQuery(
      ...
      for {
        ...
        solutions <- IO.blocking(
          asScala(execution.execSelect()).toList
        ) // orElse needed too
  ...
  private def runVersion2UsingResource = {
    val program: IO[Option[TravelGuide]] =
      connectionResource.use(connection => {
        val wikidata = getSparqlDataAccess(execQuery(connection))
        Version2.travelGuide(
          wikidata,
          "Yosemite"
        ) // this will not leak, even if there are errors
      })
  ...
  // Resource has map!
  val queryExecResource: Resource[IO, String => IO[List[QuerySolution]]] =
    connectionResource.map(execQuery)
  val dataAccessResource: Resource[IO, DataAccess] =
    connectionResource.map(connection =>
      getSparqlDataAccess(execQuery(connection))
    )

  private def runVersion2WithMappedResource = {
    unsafeRunTimedIO(
      dataAccessResource.use(dataAccess =>
        Version2.travelGuide(dataAccess, "Yosemite")
      )
    )
  }
  ...
  private def runVersion3 = {
    unsafeRunTimedIO(
      dataAccessResource.use(dataAccess =>
        Version3.travelGuide(dataAccess, "Yellowstone")
      )
  ...
  /** STEP 9: make it faster we don't have to execute queries, we can cache them
    * locally
    */
  def cachedExecQuery(
      connection: RDFConnection,
    ...
    unsafeRunTimedIO(
      connectionResource.use(connection =>
        for {
          ...
          dataAccess = getSparqlDataAccess(
            cachedExecQuery(connection, cache)
  ...
  /** BONUS: make it resilient let's fail fast if requests take too long
    * (timeout 30s)
    */
    ...
    unsafeRunTimedIO(
      connectionResource.use(connection =>
        for {
          ...
          cachedSparql =
            getSparqlDataAccess(
              cachedExecQuery(connection, cache).map(
                _.timeout(30.seconds)
              ) // note that we map over a functions result.
  ...
  /** Helper function that runs the given IO[A], times its execution, prints it,
    * and returns it
    */
